...
##Set filepaths ===================================================================================
#path to fcs files
#(modify here if you want to specif another folder -- "/home/...")
FCS.FILEPATH <- getwd()
...
md.fileNames <- as.data.frame(cbind(sampleNames(fs), 
                                    ...
                                    c(rep(0,length(sampleNames(fs)))),
                                    c(rep(0,length(sampleNames(fs))))))
colnames(md.fileNames) <- c("file_name", "stage", "genotype")
...
md.params <- as.data.frame(cbind(unlist(md.params.metal),unlist(md.params.ab),md.params.clust,
                                 md.params.plot))
colnames(md.params) <- c("Metal", "Antigen", "Clustering", "Plotting")
