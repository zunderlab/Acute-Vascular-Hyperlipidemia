...
SUBSAMPLES <- 2500 #change this value if you want subsampling. Number of cells per file
...
if (is.null(ASINH.FACTOR) == TRUE) {
  ...
  asinh.factor <- rep(ASINH.FACTOR,length(transform.markers))
}
...
if (is.numeric(SUBSAMPLES) == TRUE){
  ...
  #perform asinh transform & normalize
  concat.exprs[,transform.markers.annotate] <- sapply(seq(length(asinh.factor)),function(x){
    asinh(10^4/5)/asinh(10^4/asinh.factor[x])*asinh(concat.exprs[,x]/asinh.factor[x])})
