...
#exclude clusters 1,6
concat.transformed <- concat.transformed[which(clusters !=1 & clusters != 6),]
clusters <- clusters[clusters!=1 & clusters != 6]
#get abundances
...
p_vals <- cbind(WT_4wk_chow_p,WT_1wk_WD_p,WT_4wk_WD_p)
...
#add cluster 1 values
plotting_vals <- rbind(c(0.46,3.03),plotting_vals[1:4,],c(1.53,2.12),plotting_vals[5:6,])
...
custom_dist <- function(x){dist(rank(x) %>% matrix(nrow = clusters %>% unique %>% length + 2))}

#plot
#png("WT comparison heatmap_control_2_removed_replaced.png",
    #width = 1,height = 1,units = "in",res = 166
#    )
pdf("WT comparison heatmap_control_removed_replaced.pdf"
    )
heatmap.2(plotting_vals,Colv = FALSE,distfun = custom_dist,col = color_heat,breaks = col_breaks,
          key = FALSE,trace = "none"#,hclustfun = function(x) hclust(x, method="mcquitty"),
          #lhei = c(1,10),lwid = c(1,10),cexRow = 1,cexCol = 1,margins = c(0,0)
          )
dev.off()

#output p values
colnames(p_vals) <- c("WT_4wk_chow","WT_1wk_WD","WT_4wk_WD")
#rownames(p_vals) <- c("Diff-high","CD74+","VCAM+","Diff-mid","Diff-low","Diff-mid","CD40+","CXCL12+")
rownames(p_vals) <- sort(unique(clusters))
colnames(p_vals_corrected) <- c("WT_4wk_chow","WT_1wk_WD","WT_4wk_WD")
#rownames(p_vals_corrected) <- c("Diff-high","CD74+","VCAM+","Diff-mid","Diff-low","Diff-mid","CD40+","CXCL12+")
rownames(p_vals_corrected) <- sort(unique(clusters))
write.csv(p_vals_corrected,"WT_p_values_vs_control_1_6_removed.csv") #use write.csv instead of fwrite to keep rownames