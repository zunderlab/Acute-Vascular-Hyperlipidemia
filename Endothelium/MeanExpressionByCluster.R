...
clusters_in <- read.clusters(INPUT.FOLDER,CLUSTERS.FILENAME)

cells_compare <- data.frame(cells_in,clusters_in)
colnames(cells_compare)[ncol(cells_compare)] <- "Cluster"

#get means for each cluster within each file
means <- lapply(1:max(cells_compare$File),function(this_file){
  lapply(clusters_in %>% unique %>% sort,function(this_cluster){
    apply(cells_compare %>% dplyr::filter(File == this_file,Cluster == this_cluster) %>%
            select(),2,mean)
    })
  })
...
means_plotting <- dplyr::filter(means_plotting,condition %in% c("1 wk chow","1 wk WD","4 wk WD"))
...
for(this_marker in rownames(means)){
  ggsave(paste0(this_marker,".png"),ggplot(means_plotting,aes_string(x = "condition",y = this_marker)) + geom_boxplot() + geom_point() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(),axis.line = element_line(colour = "black"),axis.title = element_blank(),axis.text.x = element_text(angle = 270,hjust = 0,vjust = 0.5)) + guides(colour = guide_legend(override.aes = list(shape=15, size=8))),width = 1.8,height = 1.8)
}
