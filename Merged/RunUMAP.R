...
INPUT.FOLDER <- getwd()
OUTPUT.FOLDER <- getwd()
CONCATTRANSFORMED.FILENAME <- "/concat_transformed_by_Cluster_1000.csv"
PANEL.FILENAME <- "/panel.csv"
UMAP.LAYOUT.FILENAME <- "UMAP_layout_1000.csv" 
UMAP.INDEXES.FILENAME <- "UMAP_knn_indexes_1000.csv"
UMAP.DISTANCES.FILENAME <- "UMAP_knn_distances_1000.csv"
UMAP_N_NEIGHBORS <- 15
UMAP_MIN_DIST <- 0.01 #UMAP default is 0.1
...
concat.exprs <- read.concat.transformed(INPUT.FOLDER,CONCATTRANSFORMED.FILENAME)
...
## Prevent knn distance = 0 =======================================================================
if (is.na(MIN_KNN_DIST) == 1){
  umap.out$knn$distances[,2:ncol(umap.out$knn$distances
  )][which(umap.out$knn$distances[,2:ncol(umap.out$knn$distances
  )]==0)] <- 
    min(umap.out$knn$distances[umap.out$knn$distance>0])
} else {
  umap.out$knn$distances[,2:ncol(umap.out$knn$distances
  )][which(umap.out$knn$distances[,2:ncol(umap.out$knn$distances
  )]==0)] <- 
